...
export const ProductList = () => {
  ...
  // Quick Add State
  const [quickAddBrandModalOpen, setQuickAddBrandModalOpen] = useState(false);
  const [quickAddCategoryModalOpen, setQuickAddCategoryModalOpen] =
    useState(false);
  const [newQuickBrandName, setNewQuickBrandName] = useState("");
  ...
  const handleQuickAddBrand = async (e) => {
    e.preventDefault();
    if (!newQuickBrandName.trim()) return;
    setQuickAddLoading(true);
    try {
      const newBrand = await brandService.create({ name: newQuickBrandName });
      setBrands([...brands, newBrand]); // Optimistic update or just append
      setFormData({ ...formData, brandId: newBrand.id });
      setQuickAddBrandModalOpen(false);
      setNewQuickBrandName("");
      setToast({
        isOpen: true,
  ...
  return (
    <div className="flex flex-col h-full max-w-full overflow-hidden">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <div className="flex justify-between items-center mb-1">
                ...
                <button
                  type="button"
                  onClick={() => setQuickAddBrandModalOpen(true)}
      ...
      <Modal
        isOpen={quickAddBrandModalOpen}
        onClose={() => setQuickAddBrandModalOpen(false)}
        title="Quick Add Brand"
      >
        <form onSubmit={handleQuickAddBrand} className="space-y-4">
          <Input
            label="Brand Name"
            value={newQuickBrandName}
            onChange={(e) => setNewQuickBrandName(e.target.value)}
          ...
          <div className="flex justify-end gap-3 pt-2">
            <Button
              type="button"
              variant="outline"
              onClick={() => setQuickAddBrandModalOpen(false)}
            >
              Cancel
            </Button>
            <Button type="submit" disabled={quickAddLoading}>
              {quickAddLoading ? "Saving..." : "Save Brand"}
            </Button>
          </div>
        </form>
      </Modal>
